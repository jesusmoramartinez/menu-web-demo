...
export function KitchenTicket({ order, now, index, onDone }: KitchenTicketProps) {
  const min = minutesSince(order.sentToKitchenAt ?? order.createdAt, now)
  const u = urgency(min)
  const hasNotes = order.items.some((i) => i.notes)

  return (
    <article className={`flex flex-col rounded-2xl border-t-8 ${u.border} bg-white shadow-md`} aria-label={`Comanda mesa ${order.table}, ${u.label}`}>
      ...
      <div className="p-3">
        <Button variant="success" size="lg" full icon={<CheckCheck size={20} />} onClick={onDone} className="shadow-lg shadow-emerald-600/30">
