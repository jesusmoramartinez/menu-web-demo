...
import type { MenuItem, MenuTag } from '@/types/domain'
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {item.tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {item.tags.map((t) => (
              <span key={t} className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${TAG_STYLES[t]}`}>
                {t}
              </span>
            ))}
          </div>
        ...
        <div className="mt-auto flex items-center justify-between pt-2">
          <span className="text-base font-bold text-stone-900">{formatPrice(item.price, RESTAURANT.currency)}</span>
          <button
            type="button"
            onClick={handleAdd}
            aria-label={inCart ? `Agregar otro ${item.name} (${inCart.qty} en carrito)` : `Agregar ${item.name}`}
          ...
          >
            {inCart ? <Check size={16} aria-hidden="true" /> : <Plus size={16} aria-hidden="true" />}
